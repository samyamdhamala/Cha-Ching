...
class AdminDashboard(BaseDashboard):
    def __init__(self, parent, controller):
        ...
        logout_btn = ttk.Button(self, text="Logout", style="TButton", command=self.logout)
        logout_btn.place(relx=0.95, rely=0.05, anchor="ne")

        wrapper = ttk.Frame(self.content_frame)
        wrapper.pack(pady=30)

        ttk.Label(wrapper, text="Admin Dashboard", font=("Segoe UI", 20, "bold")).pack(pady=(0, 20))

        ttk.Label(wrapper, text="New Category Name:").pack()
        self.cat_name = ttk.Entry(wrapper, width=30)
        self.cat_name.pack(pady=5)
        self.cat_name.insert(0, "New Category Name")

        create_btn = ttk.Button(wrapper, text="Create Category", style="Accent.TButton", command=self.create_category)
        create_btn.pack(pady=10)

        ttk.Label(wrapper, text="Available Categories:").pack()
        self.cat_list = ttk.Combobox(wrapper, width=30)
        self.cat_list.pack(pady=5)

        delete_btn = ttk.Button(wrapper, text="Delete Category", style="Danger.TButton", command=self.delete_category)
